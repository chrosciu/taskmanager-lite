...
import eu.chrost.taskmanager.infrastructure.jpa.team.JpaSimpleTeam;
...
class JpaUser {
    ...
    User toUser() {
        ...
        user.setTeams(teams.stream().map(t -> new SimpleEntity(t.getId())).collect(Collectors.toCollection(ArrayList::new)));
        return user;
    }
}
